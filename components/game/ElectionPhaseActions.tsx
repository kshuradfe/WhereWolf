...
export default function ElectionPhaseActions({
    ...
    if (submitted) {
      return (
        <div className="space-y-1.5">
          ...
          {candidates.length > 0 && (
            <p className="text-[10px] text-amber-300/70">当前候选人：{candidates.map(getName).join(", ")}</p>
          )}
        </div>
      );
    }
    return (
      <div className="space-y-1.5">
        <div className="p-2.5 bg-amber-900/30 rounded-xl border border-amber-500/20">
          ...
          <p className="text-amber-100/70 text-[11px]">是否参与竞选警长？票多者当选。</p>
          {candidates.length > 0 && (
            <p className="text-[10px] text-amber-300/60 mt-1">已报名：{candidates.map(getName).join(", ")}</p>
          )}
        </div>
  ...
  // SPEAKING — no chat, just candidate status + withdraw button
  if (electionState === ElectionStateEnum.SPEAKING) {
    return (
      <div className="space-y-1.5">
        <div className="p-2.5 bg-amber-900/30 rounded-xl border border-amber-500/20">
          <p className="text-amber-200 text-xs font-semibold mb-1">🌟 竞选警长 — 发言阶段</p>
          <p className="text-amber-100/70 text-[11px]">候选人：{candidates.map(getName).join(", ")}</p>
          {isCandidate && (
            <p className="text-yellow-300/80 text-[10px] mt-1">你是候选人，可发言或「退水」放弃竞选。</p>
          )}
        </div>
        ...
        {submitted && isCandidate && (
          <p className="text-white/40 text-[10px] text-center">你已退水，不再参与竞选。</p>
        )}
      </div>
    );
  }
  ...
  if (electionState === ElectionStateEnum.VOTING || electionState === ElectionStateEnum.PK) {
    ...
    if (isCandidate) {
      return (
        <div className="p-2.5 bg-amber-900/30 rounded-xl border border-amber-500/20">
          ...
          <p className="text-amber-100/70 text-[11px]">你是候选人，无法投票。等待警下玩家投票...</p>
          <p className="text-amber-300/60 text-[10px] mt-1">候选人：{candidates.map(getName).join(", ")}</p>
        </div>
      );
    }
    if (submitted) {
    ...
    return (
      <div className="space-y-1.5">
        <div className="p-2.5 bg-amber-900/30 rounded-xl border border-amber-500/20">
          <p className="text-amber-200 text-xs font-semibold mb-1">
            {isPK ? "🌟 PK决选 — 投票" : "🌟 警长竞选 — 投票"}
          </p>
          <p className="text-amber-100/70 text-[11px]">
            {isPK ? "上轮平票，进行PK决选。" : "选择一名候选人投票。"}
          </p>
          <p className="text-amber-300/60 text-[10px] mt-1">候选人：{candidates.map(getName).join(", ")}</p>
        </div>
        <Button
